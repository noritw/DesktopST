...
### 授權（License）
採作者**自訂條款**（詳見作者網站或 repo 內授權文件），核心取向如下：
- 公開全文 **https://nori.tw/DeST/license.html**（設定「關於」以瀏覽器開啟）。本 repo 另附 **`docs/license.html`**（打包版置於安裝目錄 `docs/`）供離線對照。
- **禁止**將本程式與官方素材以原封不動或實質等同重新打包等方式，作為商品販售或營利目的之單獨發行（未經作者同意）。
- **允許**閱讀、研究與修改程式碼，於不違反前項之前提下為之。
- **修改後**之衍生版本：若為**免費**再發布／散布，**無須事先徵詢**；若欲**販售**或為營利目的以單獨發行物形式提供，須**事先與作者聯繫並取得同意**。
- 使用者自行建立之角色卡等內容，權利歸該使用者所有。
