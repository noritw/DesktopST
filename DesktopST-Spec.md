...
> **選讀。** 新對話不要整本讀。預設只讀 [`CLAUDE.md`](CLAUDE.md)；
> 實作某功能時再開**對應章節**。文件分級見 [`docs/README.md`](docs/README.md)。
>
> 本文件為獨立規格書，不依賴對話脈絡。讀完對應章節後應能直接實作該部分。
...
#### A. 視窗行為增補
- 啟動時若角色視窗落在畫面外（常見於多螢幕切換），會自動重設到可見工作區（必要時置中）。
- App 失焦時會隱藏非角色 UI（輸入、記錄、設定）；回到互動流程再依狀態顯示。
- 點擊/拖曳角色可把角色層級提升到最上層；操作記錄/設定/輸入視窗時，輔助視窗可回到上層。

#### B. 群組對話增補
- `message:send` 已從「全角色依序回覆」改為：
  1) 先決定主回覆者（點名優先，否則輪替）；
  2) 其他角色採「是否插話」判定，僅在有補充時回覆。
- 具備近似重複內容抑制，避免兩角色連續回覆幾乎相同內容。

#### C. LLM 介接增補
- OpenAI 已採用 Responses API（非舊 chat.completions）。
- 對 `gpt-5*`、`o*` 模型預設省略 `temperature`，避免請求被拒。
- 若模型回空字串，視為錯誤處理（寫入系統錯誤訊息），不再顯示空白回覆。

#### D. 對話記錄增補
- 支援列出舊對話、載入、改名、刪除整份當前對話檔。
...
- 設定 → 介面分頁新增「**介面配色**」功能，共 12 種主題：
  - 淺色：薄荷綠（預設）、奶油黃、粉橘、粉藍綠、天藍、粉紅、薰衣草、森林（forest）、純白（white，無彩度）
  - 深色：黑白灰（dark，無彩度）、復古（sepia，茶褐）、賽博（cyber，深綠＋深藍）
...
### 授權（License）
採作者**自訂條款**（以 `docs/license.html` 全文為準，本節僅為摘要對照）：
- 公開全文 **https://nori.tw/DeST/license.html**（設定「關於」以瀏覽器開啟）。本 repo 另附 **`docs/license.html`**（打包版置於安裝目錄 `docs/`）供離線對照。
- **禁止**將本程式與官方素材以原封不動，或僅作極小變動（例如僅更換名稱、圖示、品牌識別）之方式，作為商品販售或營利目的之單獨發行（未經作者同意）。
- **允許**閱讀、研究與修改程式碼，於不違反前項之前提下為之。
- **衍生版本**：
  - 大幅修改、整體已不易辨識為本程式延伸者，作者不主張其授權形式，惟仍**不得使用相同或近似名稱／Logo／品牌識別**（含 DesktopST、DeST、桌友等）。
  - 仍保留可辨識特徵者，須於說明文件、README 或啟動畫面標註「改作自 DesktopST（作者 Nori）」並附原作連結；得免費散布而無須徵詢，但不得販售。
- **不視為**違反「商品販售／營利發行」之情形：贊助平台收入（Patreon、抖內等）、不含官方素材之第三方付費 plugin／角色包、企業內部部署自用、直播／影片廣告分潤。
- 使用者自行建立或匯入之角色卡、對話、便利貼、提醒等內容，權利歸使用者或原作者所有；第三方匯入內容之合法性由使用者自行確認。
- **第三方開源元件**（Electron、React、Tailwind 等）各以其原始授權為準，不受本條款拘束。
- 準據法：中華民國（臺灣）法律，第一審管轄法院為臺灣桃園地方法院。
...
└── 任何人可以 Fork → 產生獨立的個人複製
    ...
    ├── Fork 者若以衍生版本免費對外發布，無須事先徵詢；若欲販售，須遵守自訂條款並取得作者同意
...
LICENSE             # 自訂授權全文；建議與 docs/license.html、https://nori.tw/DeST/license.html 同步維護
