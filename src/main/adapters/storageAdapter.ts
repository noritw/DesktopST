 ...
 * 根目錄取自 `dataDir.getDataDir()`——**每次呼叫都重新取**，
 * 因為使用者可以在設定裡搬遷資料夾（`fileStore.relocateDataDir`），不能在載入時就快取。
 *
 * ⚠️ 這支**不可以** import `fileStore`：方向是 `fileStore ─→ 本檔 ─→ dataDir`。
 */
...
export const electronStorage: StorageAdapter & SyncStorageAdapter = {
  ...
  async remove(key: string): Promise<void> {
    electronStorage.removeSync(key)
  },

  async list(prefix: string): Promise<string[]> {
    return electronStorage.listSync(prefix)
  },
  ...
  readJsonSync<T>(key: string): T | null {
    const text = electronStorage.readTextSync(key)
