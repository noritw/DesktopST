...
import { initState, registerIpcHandlers, dismissAllAuxWindows, restoreDismissedAuxWindows, hasDismissedAuxWindows, getSettings, getCharacters, getActiveConversationForMobile, addDesktopCharacterDirect, removeDesktopCharacterDirect, captureScreenshotDirect, handleSendMessageFromMobile, stopSendDirect, setMobileMessageListener, setGetMobileStatusFn, setApplyMobileRuntimeSettingsFn, getConversationListDirect, getConversationsForSyncDirect, getConversationForSyncDirect, getConversationsManifestDirect, mergeConversationMessagesDirect, loadConversationDirect, createConversationDirect, renameConversationDirect, deleteConversationDirect, getConversationMemoryDirect, summarizeConversationNowDirect, updateConversationSummaryDirect, clearConversationSummaryDirect, getScenesDirect, getPersonaPresetsDirect, getWorldPresetsDirect, activatePersonaDirect, activateWorldDirect, savePersonaPresetDirect, saveWorldPresetDirect, saveScenePresetDirect, captureSceneDirect, getActiveSceneDirtyDirect, removePersonaPresetDirect, removeWorldPresetDirect, removeScenePresetDirect, setColorThemeDirect, setShowLlmBadgeDirect, setShowPersonaNameDirect, getMessageDebugDirect, getApiKeysForSyncDirect, triggerReminderSpeak, applySceneById, handleSpotifyProtocolUrl, deleteMessageDirect, editMessageDirect, setMessageEmotionOverrideDirect, resendMessageDirect, forceSpeakDirect, toggleMuteDirect, createCharacterDirect, duplicateCharacterDirect, saveCharacterDirect, deleteCharacterDirect, saveCharacterAvatarDirect, saveCharacterEmotionSpriteAndAssignDirect, importCharacterPngDirect, importCharacterJsonDirect, exportCharacterPngDirect, exportCharacterJsonDirect, buildDstPackDirect, importDstPackDirect, listLorebooksDirect, getLorebookManifestDirect, getLorebookDirect, createLorebookDirect, saveLorebookDirect, removeLorebookDirect, generateLoreEntryDirect, getLlmSettingsSummaryDirect, setLlmProviderDirect, setLlmModelDirect, setLlmEndpointDirect, testLlmConnectionDirect, setLlmExtraInstructionDirect, setLlmUtilityEnabledDirect, setLlmUtilityProviderDirect, setLlmUtilityModelDirect, setLlmApiKeyDirect, setLlmChatLimitsDirect, getMemorySettingsDirect, setMemorySettingsDirect, listMobileModuleTogglesDirect, setMobileModuleEnabledDirect, getWeatherSettingsDirect, getWeatherSyncSettingsDirect, getNewsSyncSettingsDirect, setWeatherSettingsDirect, getMorningBriefingSettingsDirect, setMorningBriefingSettingsDirect, setCwaApiKeyDirect, detectWeatherLocationDirect, geocodeWeatherLocationDirect, fetchWeatherNowDirect, listRemindersDirect, createReminderDirect, saveReminderDirect, deleteReminderDirect, toggleReminderDirect, getCharacterDisplayConfigDirect, setCharacterDisplayConfigDirect, runCalendarReminderScanDirect, markCalendarSyncedDirect, getCalendarSyncFlagDirect, initCalendarSyncNag } from './ipcHandlers'
...
app.on('ready', async () => {
  ...
  void runCalendarReminderScanDirect()
  setInterval(() => { void runCalendarReminderScanDirect() }, 8 * 3600_000)
  // 上次的變動還沒同步到手機時，把每日「記得同步」提醒接回去（排程活在記憶體裡，重開就沒了）
  initCalendarSyncNag()
...
function initMobileServer(): void {
  ...
  setBridge({
    ...
    getConversationsForSync: getConversationsForSyncDirect,
    getConversationForSync: getConversationForSyncDirect,
    getConversationsManifest: getConversationsManifestDirect,
    mergeConversationMessages: mergeConversationMessagesDirect,
    ...
    getConversationMemory: getConversationMemoryDirect,
    summarizeConversationNow: summarizeConversationNowDirect,
    updateConversationSummary: updateConversationSummaryDirect,
    clearConversationSummary: clearConversationSummaryDirect,
    ...
    captureScene: captureSceneDirect,
    getActiveSceneDirty: getActiveSceneDirtyDirect,
    ...
    setShowPersonaName: (show) => setShowPersonaNameDirect(show),
    getMessageDebug: (id) => getMessageDebugDirect(id),
    ...
    setMessageEmotionOverride: (id, emotion) => setMessageEmotionOverrideDirect(id, emotion),
    ...
    duplicateCharacter: (id, name) => duplicateCharacterDirect({ id, name }),
    ...
    saveCharacterEmotionSprite: (id, emotionKey, buffer, ext) =>
      saveCharacterEmotionSpriteAndAssignDirect({ id, emotionKey, buffer, ext }),
    ...
    generateLoreEntry: (characterId, lorebookId) => generateLoreEntryDirect(characterId, lorebookId),
    ...
    setLlmEndpoint: (endpoint, provider) => setLlmEndpointDirect(endpoint, provider),
    testLlmConnection: (provider, endpoint) => testLlmConnectionDirect(provider, endpoint),
    setLlmExtraInstruction: (text) => setLlmExtraInstructionDirect(text),
    setLlmUtilityEnabled: (enabled) => setLlmUtilityEnabledDirect(enabled),
    setLlmUtilityProvider: (provider) => setLlmUtilityProviderDirect(provider),
    setLlmUtilityModel: (provider, model) => setLlmUtilityModelDirect(provider, model),
    setLlmApiKey: (provider, apiKey) => setLlmApiKeyDirect(provider, apiKey),
    setLlmChatLimits: (limits) => setLlmChatLimitsDirect(limits),
    ...
    getWeatherSettings: () => getWeatherSettingsDirect(),
    setWeatherSettings: (patch) => setWeatherSettingsDirect(patch),
    getWeatherSyncSettings: (lanDirect) => getWeatherSyncSettingsDirect(lanDirect),
    getNewsSyncSettings: () => getNewsSyncSettingsDirect(),
    detectWeatherLocation: () => detectWeatherLocationDirect(),
    geocodeWeatherLocation: (name) => geocodeWeatherLocationDirect(name),
    fetchWeatherNow: () => fetchWeatherNowDirect(),
    setCwaApiKey: (apiKey) => setCwaApiKeyDirect(apiKey),
    testCwaApiKey: (apiKey) => testCwaApiKey(apiKey),
    getMorningBriefingSettings: () => getMorningBriefingSettingsDirect(),
    setMorningBriefingSettings: (patch) => setMorningBriefingSettingsDirect(patch),
    // 提醒 CRUD（B3 階段 4）
    ...
    markCalendarSynced: () => markCalendarSyncedDirect(),
    // 角色顯示裁切（faceCrop，2026-08-25 起雙端同步）
    getCharacterDisplayConfig: () => getCharacterDisplayConfigDirect(),
    setCharacterDisplayConfig: (characterId, rect) => setCharacterDisplayConfigDirect(characterId, rect)
  })
