...
import { messageSpeakerLabel } from '../prompt/promptUtils'
import type { AppSettings, Conversation, Message, PersonaPreset } from '../types'
import type { LLMDeps } from './deps'

/**
 * 記憶摘要。原 `src/main/llm/summarizer.ts`，逐字沿用，
 * 唯一改動是 `summarizeConversation` 多收一個 `deps` 往下傳給 `chatWithLLM`。
 *
...
}, deps: LLMDeps): Promise<SummarizeConversationResult | null> {
