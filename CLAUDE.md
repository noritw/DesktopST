...
| **飲食記錄 App：桌面小工具（B9b 一部分）** | **已實作，自動測試通過，真機部分驗證**（2026-08-21）。`docs/nutrition-widget-plan.md` §9 七步全做完；Pixel 10a 裝機後 App 啟動、三個深連結（`daily`／`photo`／`quick-entry`）導覽、重新整理 broadcast 零實例不 crash 都驗過，**唯獨沒把小工具真的拖上主畫面看排版**（owner 的主畫面被自訂桌布/小工具塞滿，找不到安全的空白處長按，留給 owner 自己找時間放）。專案原本純 Java，這次補了 Kotlin 工具鏈，版本要對齊 `@capgo/capacitor-health` 已在用的 2.4.10（混版會炸 metadata 不相容）。點擊走 `@capacitor/app` 標準深連結模式（`custom_url_scheme` 資源＋`getLaunchUrl()`／`appUrlOpen`），存檔／App 離開前景觸發更新則靠新寫的最小外掛 `NutritionWidgetBridgePlugin`（JS 沒辦法直接發 Android broadcast）。細節見 `TODO.md` §3 對應條目。 |
...
| **下一步** | **看根目錄的 [`TODO.md`](TODO.md)** —— 待辦的唯一入口，狀態以那份為準。**2026-08-17 owner 插隊**：飲食熱量模組（B9a MVP）自用優先，開工指令在 `docs/nutrition-module-kickoff.md`（整份可讀，照著做就能開工）。**2026-08-18**：B9a 這輪的 4 個 UI 微調已做完（見 `progress-log.md` 同日條目）；Health 讀（B9-Health-lite）已於 **2026-08-19 完工並真機驗證通過**（見上一行）。**接下來排 B9b**（新食物拍照／標示 LLM；桌面小工具；本機報表頁）——owner 已確認先做 LLM 拍照估價，小工具與報表頁排在其後。S2 相關待辦（提醒同步等）暫緩，等飲食模組告一段落再回頭。 |
