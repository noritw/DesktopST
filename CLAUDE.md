...
| **下一步** | **Pixel 10a 真機驗證 S2 的三塊比對畫面 → 照 `docs/handoff/real-device-checklist.md` 勾**（23 條，已把 M4 §7 九條、M5 §8.6 六條＋`weather.polish`、對話同步 §8.4 八條抄在一起，含測試前準備）。最重要的是對話同步第 1 條：同一則推兩次不能長出第二份。驗過後 S2 就完整了；之後做階段 7 正式 APK／散布；v0.4.0 另需真機煙測（配色／新聞泡泡／遙控）。 |
