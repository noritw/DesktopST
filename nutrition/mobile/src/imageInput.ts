...
/**
 * Android 上某些「用相簿旋轉過」的照片會讓 `createImageBitmap` 直接丟
 * InvalidStateError 讀不出來（owner 2026-08-28 回報 Pixel 10a、Google 相簿；
 * 其他 App 也常見同一類問題，不是 DeST 獨有）。退回用 `<img>` 解碼一次：
 * 瀏覽器對 `<img>` 的容錯度比 `createImageBitmap`高，而且天生會套用 EXIF 方向，
 * 副作用是連原本「沒壞但方向讀反」的照片也一併修正。
 */
