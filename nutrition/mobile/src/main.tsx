...
function combineDateAndTime(baseMs: number, hhmm: string): number {
...
function App(): React.JSX.Element {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      const handle = await CapacitorApp.addListener('backButton', () => {
        ...
        if (current === 'foodForm') { setView('library'); return }
        if (current === 'library' || current === 'mealEditor' || current === 'profile') { setView('daily'); return }
  ...
  async function runAction(action: (session: NutritionSession) => Promise<void>): Promise<void> {
    setSaving(true)
    try {
      const session = await NutritionSession.boot(nutritionMobileStorage)
  ...
  function logMeal(foodItem: FoodItem): void {
    void runAction(async (session) => {
      const now = nowOnDate(selectedDate)
  ...
  async function removeFoodPhoto(index: number): Promise<void> {
    ...
    if (key) await nutritionMobileStorage.remove(key)
  ...
  function saveMealEdit(scope: 'meal' | 'food'): void {
    ...
    void runAction(async (session) => {
      ...
      if (!current) return
      const eatenAt = combineDateAndTime(current.eatenAt, mealTime)
      if (scope === 'food') {
  ...
  function saveProfile(applyTdee: boolean): void {
    ...
    const tdeeEstimate = calculateTdeeKcal({ heightCm, weightKg, ageYears, activityLevel: profileActivity })
    ...
    }).then(() => setProfileKcal(String(dailyKcalLimit)))
  }
    ...
    return (
      <main className="shell">
        ...
        <section className="food-form">
          ...
          <button type="button" className="primary" disabled={saving || !foodDraft.name.trim()} onClick={saveFoodDraft}>
    ...
    return (
      <main className="shell">
        ...
        <section className="food-form">
          ...
          <button type="button" className="primary" disabled={saving} onClick={() => saveProfile(true)}>計算並套用 TDEE</button>
        </section>
      </main>
    )
  }
  ...
  return (
    <main className="shell">
      ...
      <button type="button" className="primary full-width" disabled={saving} onClick={() => setQuickEntryOpen((open) => !open)}>
