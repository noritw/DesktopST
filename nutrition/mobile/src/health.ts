...
export const nutritionHealthAdapter: HealthAdapter = {
  ...
  async readSnapshot(): Promise<HealthSnapshot> {
    ...
    const [weightSamples, bodyFatSamples, calorieSamples] = await Promise.all([
      safeReadSamples(Health, { dataType: 'weight', startDate: recentStartIso, endDate: nowIso, limit: 5, ascending: false }),
      safeReadSamples(Health, { dataType: 'bodyFat', startDate: recentStartIso, endDate: nowIso, limit: 5, ascending: false }),
    ...
    return {
      ...
      bodyFatPercent: latestBodyFat?.value,
