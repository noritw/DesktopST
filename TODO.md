...
- [ ] Android 桌面小工具（DeST 主 App）→ **規格已重新定案（2026-08-22，
      ...
      文件 §4.1
- [ ] **手機版對話記錄換表情＋手動指定表情＋手機新增表情圖片** → **已實作三輪
      修正，自動測試通過（`npm run typecheck`／`npm test` 皆過），owner 前三次
      實機驗證都回報失敗，第四次驗證尚未進行**（2026-08-23）。
      ...
      **這次修正之前已經生成的舊訊息不會自動修好**（`emotion` 欄位還是舊的
      ...
      決議已被 owner 2026-08-22 推翻，見計畫書開頭說明。**下一步：owner 第四次
      實機驗證**——這次應該才是真的解決跨裝置同步後表情消失的問題。
