...
### 1.1 S2 同步真機驗證（23 條）→ 這是目前的第一順位

程式全部寫完了、自動測試全過，**但一條真機都還沒驗**。清單在
**[`docs/handoff/real-device-checklist.md`](docs/handoff/real-device-checklist.md)**
（含測試前準備與 `adb logcat` 抓法），23 條分三節：

- [ ] 一、S2 M4 比對畫面（9 條）
- [ ] 二、S2 M5 設定同步（6 條 ＋ `weather.polish` 補驗 1 條）
- [ ] 三、對話同步（8 條）

> **時間有限就先測這一條**：第三節第 1 條——手機開一則新對話 → 切到遙控 → 同步 →
> **再切一次不能長出第二份**。S2 M3 當初就是栽在重複增生，最後靠一次性腳本清電腦
> 資料才收拾掉，那份資料在你電腦上是真的會被弄髒的。
