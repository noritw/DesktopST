...
## 2.9 連結閱讀：貼網址讓角色讀內文（2026-09-13 owner 指定）→ ✅ 已實作，**基本讀取桌面＋手機皆實測通過**
...
「能讀的連結和桌面版差不多」——**基本讀取兩邊都過關**。
**仍未驗**：YouTube 說明欄（當天稍晚才做）、模組開關關掉後不再抓、
情境覆蓋、手機用行動網路時的流量與延遲。清單在該文件 §7。

範圍**只做公開網頁**——需要登入才看得到的頁面（社群貼文等）一律讀不到，
而且是設計如此：抓取從主行程／原生層發出，那裡沒有使用者瀏覽器的 cookie。
社群站（X／FB／IG／Threads／YouTube…）連抓都不抓，直接回報讀不到。
...
- [ ] 飲食熱量模組其餘分期 → `docs/future-nutrition-module.md` §6。
      ...
      **同日 owner 對 B9c 三項逐一定調，動工前先讀
      ...
      `npm test`（81 檔、1063 項）皆過，**真機驗證留給 owner**。
