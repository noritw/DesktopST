...
>
> 狀態：**A／B1／B2／B2.5–B2.7 完成；B3 手機 UI 0–9＋資訊架構完成；B6 遙控 UI 完成；手機獨立版 W1–W3 完成。**
> **目前下一步：B3 階段 7（正式 APK／散布）**；個人新聞報／地方新聞併關鍵字組等功能已落地，且已補真機驗證。
> 基準版本：v0.4.x｜討論日期：2026-08-02｜最後更新：**2026-08-15**
...
| **S2 手動雙向同步** | 明確的「推送 / 拉取 / 合併」按鈕 ＋ **差異預覽**，使用者按下才動 | 2–3 週 | MVP 必做；**觸發時機已改，見下方註** |
...
> **S2 觸發時機修正（owner 2026-08-09）**：S2 原本設計成「設定頁裡的推送／拉取按鈕」，
> 但 owner 的實際形態是**獨立為常態、遙控為加值**，兩邊是交替使用而非長期並行。
> 因此 S2 第一階段改為在**切換模式的那一刻**觸發，方向由「剛離開的模式」決定
> （獨立→遙控＝問手機資料要不要帶到電腦，反之亦然）。
> 上表的合併規則不變，仍是本文的權威來源。
> **實作設計：`mobile-mode-switch-sync.md`**（含基準結構、要新增的兩支端點、分階段）。
