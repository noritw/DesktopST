...
### 1.1 已完成（`feat/mobile-standalone`，11 個 commit）
...
| 尾巴 | 現況 | 風險 |
|---|---|---|
| **實機從未驗證** | B1 收尾動到了聊天主線（`llm/` 四塊）。分支未合併，owner 平時開的是 `main` 上的舊版 | 若有回歸，會在 B3 做到一半才發現，屆時難以判斷是新寫的還是舊搬的 |
| **Capacitor 從未執行** | 刻意沒做 `cap add android`、沒產 APK | 手機殼一行都沒跑過。若殼本身有問題，會在寫完數週 UI 後才知道 |

**兩者都必須在 B3 開工前收掉**，處理方式見 §6 與 §5。
...
| **`sources.ts` 的手機方案** | B4，但宜早決定 | 用了 `crypto.createHash`（Node 專屬）與 `rss-parser`（需確認 WebView 可跑）。若新聞在手機 MVP 範圍內，這個決定會影響 B3 的資料層 |
...
| # | 議題 | 選項 |
|---|---|---|
| 1 | B2.6（Lorebook 桌面 UI）現在做還是 B3 後 | 現在做＝桌面早點能用；延後＝早點進 B3 |
| 2 | 角色印象是否先定型別（半天） | 定了 B3 不會撞到資料結構；不定則完全延後 |
| 3 | Android keystore 的產生與保管 | 見 roadmap §11.3。**AI 不會自行產生** |
| 4 | 新聞是否納入手機 MVP | 影響 `sources.ts` 的手機方案要多早決定 |
