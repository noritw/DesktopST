...
**三、`crypto.createHash`** —— ✅ 已於 2026-08-04 解掉，但**不是**用這裡原本寫的方式，
理由見 §8 該列：id 已進使用者資料，只能維持與 SHA-1 完全相同的輸出。
...
| 依賴 | 問題 | 現況 |
|---|---|---|
| `crypto.createHash` | Node 專屬，手機端沒有 | ✅ **已解（2026-08-04）**。⚠️ **沒有照這裡原本寫的「換一個 hash」做** —— 該 id 已被寫進 `seenIds` 與新聞報釘選當鍵，換演算法會讓那些狀態靜默失效。改為自寫純 JS **SHA-1**（`core/util/sha1.ts`），與 Node 逐位元組相同、零遷移 |
