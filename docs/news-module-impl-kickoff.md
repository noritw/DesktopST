...
- [ ] 新聞模組可在設定中**啟用 / 停用**;停用時其他功能無影響。
- [ ] 興趣設定面板:打字→Enter→變標籤,點標籤循環常聊/普通/偶爾,**全程不需碰 RSS 字眼**。
- [ ] 黑名單命中(title/summary/tags/category/source 任一含關鍵字)即不出現;來源/整類排除生效。
- [ ] 語言三選一:translate 模式下,日文/簡中新聞由角色用繁體中文轉述,不貼原文。
- [ ] 接 `https://news.nori.idv.tw/news.json` 為 json 來源能正常抓取與去重(同一 id 不重複聊)。
- [ ] `keyword` 來源能正確組出 Google News RSS 並抓到結果。
- [ ] **按「說點什麼」**:偶爾/每次模式會抽新聞來聊,點掉後再按能抽下一則。
- [ ] 提醒觸發預設關;開啟後能定時抽。
- [ ] 地方新聞:首開帶入定位縣市,可加台北/新北/台南多個、各自權重、各自刪。
- [ ] 破圈:可開關、可設頻率,從熱搜抽且仍受黑名單過濾。
- [ ] 「不想聽這個」按鈕:純略過 / 勾選封鎖關鍵字或來源 兩種行為正確;**無長按**。
- [ ] 學習權重可一鍵重置。
- [ ] 新聞 LLM 走**輔助模型**(未啟用分流時沿用主模型);角色輸出是聊天口吻、繁體中文,不像播報。
